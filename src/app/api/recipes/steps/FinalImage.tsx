...
import { useFinalImage } from '@/app/api/hooks/useFinalImage'
import { useRecipePreview } from '@/app/api/hooks/useRecipePreview'
import { IconAlertCircle, IconLoader, IconImage } from '@/components/ui/icons'
...
export function FinalImage({ data, onChange, onNext, onBack }: StepComponentProps) {
  ...
  const handleImageUpdate = async (imageUrl: string | undefined) => {
    ...
    try {
      onChange(updates)
      await updatePreview('finalImage', updates)
